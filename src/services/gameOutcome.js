...
async function calculateOutcome(userId, betAmount, gameType, gameId) {
  ...
  const updatedPools = await getPoolBalances(gameId);
  ...
  if (roll >= smallCutoff && updatedPools.medium_win >= betAmount * 2) {
  ...
  if (roll >= lossCutoff && updatedPools.small_win >= betAmount * 0.5) {
    ...
    if (globalCap > 0 && globalCap <= betAmount * 1.5) {
